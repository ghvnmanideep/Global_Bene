...
class AdvancedTopKRecommender:
    ...
    def _load_votes_df(self) -> pd.DataFrame:
        """
        Read userinteractionlogs collection for vote actions.
        action: 'vote_post_up' -> value = 1
        action: 'vote_post_down' -> value = -1
        """
        try:
            rows = []
            coll = self.db.get_collection("userinteractionlogs")
            for doc in coll.find({
                "action": {"$in": ["vote_post_up", "vote_post_down"]},
                "targetType": "post"
            }):
                uid = str(doc.get("userId", ""))
                tid = str(doc.get("targetId", ""))
                action = doc.get("action", "")
                value = 1 if action == "vote_post_up" else -1
