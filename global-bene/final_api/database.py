    ...
    def get_users(self, filter_query=None):
        try:
            ...
            for idx, row in users_df.iterrows():
                ...
                # num_posts
                num_posts = self.db['posts'].count_documents({'author': user_id})
                users_df.at[idx, 'num_posts'] = num_posts

                # num_comments
                num_comments = self.db['comments'].count_documents({'author': user_id})
                users_df.at[idx, 'num_comments'] = num_comments

                # communities_followed
                communities = list(self.db['userinteractionlogs'].distinct('targetId', {
                    'userId': user_id,
                    'action': 'join_community',
                    'targetType': 'community'
                }))
                users_df.at[idx, 'communities_followed'] = communities
